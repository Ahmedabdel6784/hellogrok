...
Grok Build 会在 `/model` 切换后重放历史推理项，其中可能包含其他模型家族无法验证或解密的服务商加密状态，从而造成确定性请求拒绝和反复重试。hellogrok 现在只删除已知来自不同渠道、协议、线上模型或上游端点的完整加密推理项；可见消息、工具调用、工具结果、搜索历史和未加密推理保持不变。

不透明推理来源会跨代理重启保存在有容量上限的私有索引中，文件只包含 SHA-256 摘要。同一签名域的状态继续透传；旧会话中的未知状态首次也保持透传。只有上游返回结构化签名或解密拒绝时，代理才会执行一次排除不透明推理的重放；再次拒绝会被标记为不可重试。
